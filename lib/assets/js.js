$(document).ready(function () {
	...
	socket.on("update", function (msg, clients) {
		if (ready) {
			...
			$('.chat').append('<li style="margin-bottom: 4px;" class="info"> ' + msg + ' </li>')
			if (clients != null) {
				$('.usuarios').empty().append('');
				for (var k in clients) {
					$('.usuarios').append('<div class="row sideBar-body "><div class="col-sm-3 col-xs-3 sideBar-avatar"><div class="avatar-icon"><img src="lib/styles/user.jpg"></div></div><div class="col-sm-9 col-xs-9 sideBar-main ">' + clients[k] + '<div class="row"></div></div></div>');
